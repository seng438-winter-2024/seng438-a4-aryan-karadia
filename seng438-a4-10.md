...
## Test Case #5: LogInWithValidUser

This simple test cases tests whether or not the GUI is updated properly when an invalid email address is inputted when trying to log in. The path of the script is:

1. Click `Sign In` button
2. Enter valid email id in to `userid` field
3. Enter valid password id in to `passwordid` field
4. Attempt to sign in by clicking `Continue` button
5. Assert sign in occurs and GUI continues
